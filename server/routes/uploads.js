...
const buildFileArray = (filesArray = []) => filesArray.map(buildFileMeta).filter(Boolean);
  ...
  async (req, res) => {
    try {
      ...
      if (parsedLinks.length) {
        user.portfolioLinks = Array.from(new Set([...user.portfolioLinks, ...parsedLinks]));
      }

      const addFiles = (fieldName, storageField) => {
        const incoming = req.files[fieldName];
        if (incoming && incoming.length) {
          user[storageField] = [...user[storageField], ...buildFileArray(incoming)];
        }
      };
