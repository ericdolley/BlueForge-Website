...
const AuthPanel = () => {
  ...
  const handleSignup = async event => {
    ...
    try {
      ...
      setFeedback('Signup created. Check your inbox for the verification link.');
    ...
    <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-2xl shadow-slate-900/60 backdrop-blur">
      <div className="flex items-center justify-between">
        <button
          ...
          className={`text-sm font-semibold uppercase tracking-[0.2em] ${mode === 'login' ? 'text-white' : 'text-slate-400'}`}
        >
          Login
        </button>
        <button
          onClick={() => setMode('signup')}
          className={`text-sm font-semibold uppercase tracking-[0.2em] ${mode === 'signup' ? 'text-white' : 'text-slate-400'}`}
        >
          Sign Up
        </button>
      </div>
